" vim-conceal里的
    ...
    syn    match pyOperator "\<\%(math\.\)\?prod\>" conceal cchar=∏
    syn    match pyOperator "\( \|\)\*\*\( \|\)2\>" conceal cchar=²
    syn    match pyOperator "\( \|\)\*\*\( \|\)3\>" conceal cchar=³
    syn    match pyKeyword "\<\%(math\.\)\?pi\>" conceal cchar=π

    " keywords
    syn    keyword pyOperator product conceal cchar=∏
...
" Remove the keywords. We'll re-add them below. Use silent in case the group  doesn't exist.
    ...
    syn    match Normal '\s\*\s'ms=s+1,me=e-1 conceal cchar=x
    " syntax match Normal '\v(\+|-|*|/|\%)@!\=' conceal cchar=←
    syn    match Normal '\v[^-=+*/]\zs\=\ze[^=]' conceal cchar=←
    ...
    syn    keyword Normal class conceal cchar=※
